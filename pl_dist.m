...
% Ajuste y gráfica para la primera matriz (Lab 3)
modelo1 = fit(datos_lab3(:,2), datos_lab3(:,4), 'poly1'); % Ajuste de un polinomio de primer grado (lineal)
plot(modelo1, datos_lab3(:,2), datos_lab3(:,4), 'ro'); % Graficar los datos y la curva de ajuste en rojo con círculos
hold on; % Mantener la gráfica actual

% Ajuste y gráfica para la segunda matriz (Lab 4)
modelo2 = fit(datos_lab4(:,2), datos_lab4(:,4), 'poly1'); % Ajuste de un polinomio de primer grado (lineal)
plot(modelo2, datos_lab4(:,2), datos_lab4(:,4), 'bo'); % Graficar los datos y la curva de ajuste en azul con cruces

% Etiquetas y título
xlabel('Distancia Logarítmica');
ylabel('Path Loss (dB)');
title('Curva de Ajuste - Path Loss vs. Distancia Logarítmica');

% Leyenda
legend('Lab 3 - Datos y Ajuste', 'Lab 4 - Datos y Ajuste');
...
% Coeficientes del lab3
coeficientes1 = coeffvalues(modelo1);
% Mostrar la ecuación del lab3
fprintf('Ecuación del modelo 1: y = %.4fx + %.4f\n', coeficientes1(1), coeficientes1(2));

% Coeficientes del lab4
coeficientes2 = coeffvalues(modelo2);
 % Mostrar la ecuación del lab4
fprintf('Ecuación del modelo 2: y = %.4fx + %.4f\n', coeficientes2(1), coeficientes2(2));
